...
$A_{\mathrm{in}}$ is obtained by taking the $\varpi$-adic
...
\[
  ...
  \qquad T=T_0[1/\varpi].
\]
Let $\rho:A\to A_{\mathrm{in}}$ be the canonical map.  We first show that
...
$A_{\mathrm{in}}$, this proves \eqref{eq:q2-killed-on-chart}.
...
image of $A$ is dense in $A_{\mathrm{in}}$, so continuity gives
...
norm at most $\lVert T_n\rVert_{\mathrm{in}}$.  Thus every $T_n$ is
