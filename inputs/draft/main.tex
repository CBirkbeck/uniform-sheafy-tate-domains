...
$A_{\mathrm{in}}$ is obtained by taking the separated $\varpi$-adic
...
\]
for every $n$.  Hence $\varpi^r y$ maps to zero in the separated
$\varpi$-adic completion of $A_0[X]$.  Since $\varpi$ is invertible in
...
$A_0[X]$ into $T_0$ and extends, first to the separated completions and then
...
gives a continuous map $P\to k\langle X\rangle$.  Conversely, evaluation of
...
$R_0$ and $W/\varpi$, and the hat denotes separated $\varpi$-adic
completion.  Its generic fibre is the rational domain
$|W|\leq|\varpi|$.  This description applies to our nonnoetherian formal
models because they are $\varpi$-adic and their ideal of definition
$(\varpi)$ is principal; see
\cite[Chapter~II, Section~1.1(b) and Example~6.1.7(3)]{FK}.

For $R_0=A_0$, the ring $A_0[W/\varpi]$ is a domain, but the calculation in
the proof of \cref{prop:bad-chart} shows that
\[
  Q^2\in\bigcap_{n\geq0}\varpi^nA_0[W/\varpi].
\]
Separated completion therefore kills $Q^2$, while $Q$ survives, so this chart
has nonreduced generic fibre.  The induced map on generic-fibre rings is also
not flat by \cref{prop:problems-24-28}.  For $R_0=\mathcal A_0$, the completed
