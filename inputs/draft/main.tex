...
Let $E$ be one of $B,C,D$, equipped with its coefficientwise Gauss norm, and
let $E_0$ denote its unit ball.  Let $f_1,\dots,f_m,g\in E_0$ generate the
unit ideal in $E$.  Put
\[
  T_E=E\langle T_1,\dots,T_m\rangle,
  \qquad
...
$\{\varpi^nT_{E,0}\}_{n\geq0}$ is a basis of neighbourhoods of zero.  In the
...
\]
is flat.  In fact, $E_0$ is noetherian for $E=B,C,D$, so
$E_0[T_1,\ldots,T_m]$ is noetherian and its $\varpi$-adic completion is flat
over it \cite[Tag~00MB]{Stacks}.  Inverting $\varpi$ gives the displayed flat
map.  Tensoring the polynomial Koszul complex with $T_E$, and using the
...
\section{Strong sheafiness}

\begin{theorem}\label{thm:sheafy}
The ring $A$ is strongly sheafy.  In particular, the Huber pair
$(A,A^\circ)$ is sheafy.
\end{theorem}
...
Each of $B,C,D$ is a classical strongly noetherian affinoid $k$-algebra and
is therefore sheafy by Huber's theorem \cite[Theorem 2.2]{Huber94}.  Let
...
of the continuous difference maps to $\mathcal O(U_i\cap U_j)$, and hence is
a Banach $k$-space.  The restriction map is a continuous bijection from the
Banach $k$-space $\mathcal O(U)$ to this closed subspace.  The
nonarchimedean open mapping theorem therefore makes it a homeomorphism.
This proves the sheaf condition on finite rational covers.  The
sheaf-on-a-rational-basis theorem \cite[Tag 009O]{Stacks} then gives the
all-open topological sheaf condition for $(A,A^\circ)$.

The same proof applies after adjoining $V_1,\ldots,V_s$.  The split Milnor
row extends coefficientwise, the three other vertices remain strongly
noetherian affinoid $k$-algebras, and the Koszul argument above is unchanged.
Thus $A\langle V_1,\ldots,V_s\rangle$ is sheafy for every $s\geq0$.
\end{proof}
...
\item $\mathcal A$ is a uniform, non-noetherian integral domain and
      $\mathcal A^\circ=\mathcal A_0$, where $\mathcal A_0$ is defined below.
\item The ring $\mathcal A$ is strongly sheafy; in particular, the Huber pair
      $(\mathcal A,\mathcal A^\circ)$ is sheafy.
...
The ring $\mathcal A$ is strongly sheafy.  In particular, the Huber pair
$(\mathcal A,\mathcal A^\circ)$ is sheafy.
\end{theorem}
...
continuous bijection from the Banach $k$-space $\mathcal O_X(U)$ to this
...
every weight function, so this Tate extension is sheafy.  Hence $\mathcal A$
is strongly sheafy.
\end{proof}
...
Problems~24 and 28 from commit \texttt{01116aca6}.  The strong-sheafiness
statements are taken from commit \texttt{870d0eed2} \cite{AINTLIB}.  For each
...
theorem weightedParity_not_stablyUniform_of_dvr :
    ¬ IsStablyUniform (WPAid K) := by
  ...
\end{lstlisting}

The strong-sheafiness statement is in
\href{https://cbirkbeck.github.io/uniform-sheafy-tate-domains/lean/AdicSpaces/declarations/WeightedParity.wp_tateExt_isSheafyComplete_of_dvr.html}{\texttt{WP/StrongSheafy.lean}}:

\begin{lstlisting}
theorem wp_tateExt_isSheafyComplete_of_dvr
...
Taking $w=\texttt{idWeight}$, the restricted power-series subring is
$\mathcal A\langle V_1,\ldots,V_s\rangle$.  Thus this theorem is precisely
the strong sheafiness asserted in \Cref{thm:parity-sheafy}, for the full
DVR-base scope of the paper.
