...
imply sheafiness \cite{BV,Mihara}.  They leave open the possibility of a
...
The same rational chart gives a negative answer to Problem~24 of the
\emph{Nonarchimedean Scottish Book} and an affirmative answer to the Tate
case of Problem~28; see \cref{sec:problems-24-28}.  These two consequences
have also been formalised in Lean under the weaker assumption that $k$ is a
complete ultrametric normed field equipped with a nonzero element $\varpi$
satisfying $|\varpi|<1$.  In particular, the formalisation applies over
$\mathbb Q_p$ as well as over $F((t))$.
...
restrict to zero on a rational subspace \cite[Problems~24 and 28]{Scottish}.
...
In the finite-jet construction, the canonical map to the rational localisation
kills $Q^2$ but not $Q$.  We now give a second construction in which the bad rational localisation is an
...
to $|W|\leq|\varpi|$.  This description applies to our nonnoetherian formal
...
while $Q$ survives, so this rational chart is nonreduced.  The map from $A$ to
...
from commit \texttt{d92f96504}, and the consequences for Scottish Book
Problems~24 and 28 from commit \texttt{01116aca6} \cite{AINTLIB}.  For each
example Lean proves the
...
The consequences in \cref{sec:problems-24-28} are not restricted to
Laurent-series fields.  They are formalised for any complete ultrametric
normed field $K$ equipped with a nonzero element $\varpi$ satisfying
...
The source happens to call the base parameter \texttt{F}, but it denotes $K$
itself, not the coefficient field of a Laurent-series field.  Sorry-free
instances are provided for $F((t))$ and for $\mathbb Q_p$; the latter also
gives the explicit
\href{https://cbirkbeck.github.io/uniform-sheafy-tate-domains/lean/AdicSpaces/declarations/FiniteJet.finiteJet_padic_quality.html}{$p$-adic finite-jet ring}.
...
noetherian Gauss-unit-ball input used in the sheaf proof.  A sorry-free
instance of this additional class is currently supplied for $F((t))$, but
not for $\mathbb Q_p$.  Neither of the Scottish Book conclusions uses it.
The independent \texttt{FiniteJetOver} development used for the main theorem
proves sheafiness under the complete discretely valued hypothesis of the
paper.
