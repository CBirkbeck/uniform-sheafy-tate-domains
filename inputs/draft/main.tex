...
localisation is an integral domain.  All the results in this paper are due to
ChatGPT 5.6 Sol.  Both constructions, including their sheafiness and the
rational localisations witnessing failure of stable uniformity, have been
...
All the results in this paper are due to ChatGPT 5.6 Sol.  We have aimed to
include references to related work that may have been used by ChatGPT in
finding the examples; see \cref{rem:earlier-work}.

We begin with what ChatGPT called the ``finite-jet pullback''.  This is a
...
The construction, uniformity, sheafiness, and failure of stable uniformity
for both examples have been formalised in Lean~4.  The formalisation also
includes the rational-chart calculations used above.
\Cref{sec:formalisation} records the relevant definitions and theorem
statements.
...
For $N\geq1$, let $\mathcal A_N\subset\mathcal A$ be the closed subring
supported on monomials involving only $W,U_1,\ldots,U_N$.  Put
\[
  Y_n=W^nU_n,\qquad Z_n=U_n^2,
\]
and let $\mathcal T_N\subset\mathcal A_N$ be the subring supported on even
$U_n$-exponents.

\begin{lemma}[Finite heads]\label{lem:finite-stage-normal-form}
The substitution $Z_n\mapsto U_n^2$ gives a norm-preserving ring
isomorphism
\[
  k\langle W,Z_1,\ldots,Z_N\rangle\xrightarrow{\ \sim\ }\mathcal T_N.
\]
Every $f\in\mathcal A_N$ has a parity-slice expansion
\begin{equation}\label{eq:finite-stage-normal-form}
  f=\sum_{\epsilon\in\{0,1\}^N} f_\epsilon Y^\epsilon,
  \qquad f_\epsilon\in\mathcal T_N.
\end{equation}
Consequently $\mathcal A_N$ is finite over $\mathcal T_N$ and is a strongly
noetherian Tate ring.  The inclusions between the heads and into
$\mathcal A$ preserve the norm.  Moreover, for every $f\in\mathcal A$ and
$\ell\geq0$ there are $N$ and $f_N\in\mathcal A_N$ such that
\[
...
preserve the Gauss norm.  Splitting each $U_n$-exponent according to its
parity gives \eqref{eq:finite-stage-normal-form}.  The finitely many
$Y^\epsilon$ therefore generate $\mathcal A_N$ over $\mathcal T_N$.
...
isometric.  Finally, truncating a restricted series to a sufficiently large
head gives the stated estimate and hence
\eqref{eq:A-completion-of-heads}.
\end{proof}
...
the rational chart bicontinuously with the $c_0$ tail algebra over $Q$.
After replacing $Q$ by $k\langle X\rangle$, this is the model
\eqref{eq:weighted-chart-lattice}; its sup norm is
\eqref{eq:weighted-chart-norm}, since the tail basis element of parity
$\nu$ contributes the factor $\varpi^{\omega(\nu)}$.  The head evaluation
sends $W$ to $\varpi X$.
\end{proof}
...
Let $E$ be a complete nonarchimedean normed commutative ring, let
$E^+\subset E$ be a plus ring containing the closed unit ball
$E_0=\{x:\lVert x\rVert\leq1\}$, and let $t\in E^\times$ satisfy
\[
  0<\lVert t\rVert<1,\qquad
  \lVert tx\rVert=\lVert t\rVert\lVert x\rVert\quad(x\in E).
\]
Let $\alpha=(f_1,\ldots,f_m;g)$ be a rational datum with entries in
$E_0$.  Write $d_0=g$ and $d_i=f_i$ for $i\geq1$.  Suppose that for some
$\ell\geq0$ there are $a_0,\ldots,a_m\in E_0$ with
\begin{equation}\label{eq:integral-bezout}
  t^\ell=\sum_{j=0}^m a_jd_j.
\end{equation}
Let $d_j'\in E_0$ satisfy
\begin{equation}\label{eq:small-perturbation-bound}
  \lVert d_j'-d_j\rVert\leq\lVert t\rVert^{\ell+1}
  \qquad(0\leq j\leq m),
\end{equation}
and write $\alpha'=(f_1',\ldots,f_m';g')$.  Then $\alpha'$ is a rational
datum, $\alpha$ and $\alpha'$ define the same rational subset of
$\Spa(E,E^+)$, and their completed rational localisations are
bicontinuously isomorphic by an isomorphism commuting with the canonical
maps from $E$.
\end{lemma}

\begin{proof}
The scaling hypothesis and the fact that $t$ is a unit give
$d_j'-d_j=t^{\ell+1}h_j$ with $\lVert h_j\rVert\leq1$.  Hence
\[
  \sum_j a_jd_j'
  =t^\ell(1+tH),
  \qquad H=\sum_j a_jh_j,\quad \lVert H\rVert\leq1.
\]
Since $\lVert tH\rVert<1$, the element $1+tH$ is a unit with inverse in
$E_0$.  Multiplying the $a_j$ by this inverse gives a unit-ball Bezout
relation for the primed datum; in particular, $\alpha'$ is rational.
...
\[
  |t(x)|^\ell\leq |g(x)|.
\]
The perturbations have value at most $|t(x)|^{\ell+1}<|g(x)|$, so
$|g'(x)|=|g(x)|$ and
...
rational restriction shows that their composites are the identity.  The two
restriction maps are continuous and commute with the canonical maps from
$E$, as required.
\end{proof}
...
$P=(\mathcal A_N)_\alpha$.  There is a bicontinuous ring isomorphism
...
\eqref{eq:tail-multiplication} shows that both maps are ring
homomorphisms.
...
Every rational localisation of $\mathcal A$ is bicontinuously isomorphic to a ring of the form
\begin{equation}\label{eq:arbitrary-finite-head-presentation}
  \widehat{\bigoplus}_{\mu}^{\,c_0}Pe_\mu,
\end{equation}
where $P$ is a rational localisation of some strongly noetherian Tate head
$\mathcal A_N$.
\end{corollary}
...
$\lVert d_j'-d_j\rVert\leq|\varpi|^{\ell+1}$.  By
...
of one strongly noetherian Tate head and the $P_i$ are rational localisations
of $P$.  The
...
multivariable power series over this domain.  The small-perturbation lemma
of Section~7 is the unit-ball, norm-scaling theorem formalised by
\texttt{WeightedParity.PerturbSetup}.
