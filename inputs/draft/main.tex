...
By the preceding coefficient description, $A$ consists of the same restricted
series as $A_0$, but with coefficients in $k$.  Such a series lies in $C_0$
precisely when all of its coefficients lie in $k^\circ$.  Hence
\[
  A_0=A\cap C_0.
\]
...
defines $A_{\mathrm{in}}$.  If $y\in Q^2C_0$, its constant and linear
$Q$-coefficients vanish, so $W^{-n}y\in A_0$ for every $n\geq0$.  In $G_0$
one has
...
\[
  ...
  \qquad T=T_0[1/\varpi].
\]
Using the unique expression in \eqref{eq:coefficient-A0}, define
\[
  f_0(W)+Qf_1(W)+Q^2h
  \longmapsto
