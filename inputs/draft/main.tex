...
The fact that a rational subset is unchanged by sufficiently small
perturbations of its defining functions is standard; see
\cite[Lemma~3.10]{Huber93} and \cite[Remark~2.4.7]{KL}.  We record the
following quantitative form because the explicit bound is needed below.
The identification of the completed localisations uses the universal
property of rational localisation \cite[Proposition~1.3]{Huber94}.
...
$E_0$.  Suppose that for some $\ell\geq0$ there are
$a_0,\ldots,a_m\in E_0$ with
\begin{equation}\label{eq:integral-bezout}
  t^\ell=a_0g+\sum_{i=1}^m a_if_i.
\end{equation}
Let $g',f_1',\ldots,f_m'\in E_0$ satisfy
\begin{equation}\label{eq:small-perturbation-bound}
  \lVert g'-g\rVert\leq\lVert t\rVert^{\ell+1},
  \qquad
  \lVert f_i'-f_i\rVert\leq\lVert t\rVert^{\ell+1}
  \quad(1\leq i\leq m),
\end{equation}
and put $\alpha'=(f_1',\ldots,f_m';g')$.  Then $\alpha'$ is a rational
...
$g'-g=t^{\ell+1}h_0$ and
$f_i'-f_i=t^{\ell+1}h_i$, with
$h_0,h_1,\ldots,h_m\in E_0$.  Hence
\[
  a_0g'+\sum_{i=1}^m a_if_i'
  =t^\ell(1+tH),
  \qquad
  H=a_0h_0+\sum_{i=1}^m a_ih_i\in E_0.
\]
Since $\lVert tH\rVert<1$, the element $1+tH$ is a unit with inverse in
$E_0$.  The displayed identity therefore shows that the primed entries
generate the unit ideal; in particular, $\alpha'$ is rational.

Let $x$ lie in the rational subset defined by $\alpha$.  Since
$a_0,a_1,\ldots,a_m\in E_0\subseteq E^+$,
...
argument to
\[
  a_0g'+\sum_i a_if_i'=t^\ell(1+tH)
\]
proves the reverse inclusion, since $|1+tH|=1$ at every point.
...
so assume that $g,f_1,\ldots,f_m$ lie in $\mathcal A_0$.  Choose an
integral Bezout relation
\[
  \varpi^\ell=a_0g+\sum_{i=1}^m a_if_i,
  \qquad a_0,a_1,\ldots,a_m\in\mathcal A_0.
\]
By \eqref{eq:A-completion-finite-variables}, for some $N$ there are
$g',f_1',\ldots,f_m'\in\mathcal A^{(N)}\cap\mathcal A_0$ such that
\[
  \lVert g'-g\rVert\leq|\varpi|^{\ell+1},
  \qquad
  \lVert f_i'-f_i\rVert\leq|\varpi|^{\ell+1}.
\]
By \cref{lem:small-perturbation}, the datum
$\alpha'=(f_1',\ldots,f_m';g')$ gives the same rational subset and the same
completed localisation.  Apply $\rho_N$ to the identity
\[
  a_0g'+\sum_i a_if_i'=\varpi^\ell(1+\varpi H)
\]
from the proof of that lemma.  Its right-hand side remains a unit, so
$\alpha'$ is already a rational datum in $\mathcal A^{(N)}$.  Now apply
...
It was through this process that ChatGPT 5.2 Pro found
\cref{lem:small-perturbation}.  After a couple of weeks it claimed to have a
complete proof.  The author then began the slow process of understanding
this proof, only to discover, with the help of ChatGPT 5.3, that one of its
arguments used the following assertion:
