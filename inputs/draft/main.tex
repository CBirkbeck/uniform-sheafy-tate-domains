...
case of Problem~28; see \cref{sec:problems-24-28}.
...
Both questions can be read off from the chart computed in
\cref{prop:bad-chart}.
...
\]
is norm-nonincreasing and sends $A^\circ$ into $k^\circ$.  Pulling back the
valuation on $k$ therefore gives a point of $\Spa(A,A^\circ)$.  At this point
$W$ has value zero and $\varpi$ has nonzero value, so the point lies in $U$.

Under the inclusion $A\subset C$, the element $Q^2$ is the usual nonzero
monomial.  Since the restricted Gauss norm on $A$ is multiplicative and
$\lVert Q\rVert=1$, for every $a\in A$ we have
\[
  \lVert Q^2a\rVert=\lVert a\rVert.
\]
Thus multiplication by $Q^2$ is an isometry.  In particular it is injective,
and its image is closed because $A$ is complete.  It is therefore a strict
inclusion in the sense of Problem~28.

On the other hand, \cref{prop:bad-chart} identifies the restriction map with
\[
  A\longrightarrow k\langle X,Q\rangle/(Q^2),
  \qquad W\longmapsto\varpi X,\quad Q\longmapsto Q.
\]
It follows that $Q^2$ maps to zero.  The target is nonzero.  If the rational
localisation were flat, tensoring the injection
\[
  0\longrightarrow A\xrightarrow{\,\cdot Q^2\,}A
\]
with $\mathcal O(U)$ would make multiplication by the image of $Q^2$
injective on $\mathcal O(U)$.  This map is zero, which is a contradiction.
\end{proof}
...
from commit \texttt{d92f96504} \cite{AINTLIB}.  For each example Lean proves the
...
\texttt{d92f96504f949ca43a27a817cb8d2f70b6486744}, 2026,
